...
describe('parseRequest', () => {
  ...
  it('should coerce numeric values', () => {
    ...
    expect(((result.filters!.price as any) as FilterParam).value).toBe(42.5);
  });

  it('should coerce boolean values', () => {
    const result = parseRequest({ 'filter[active]': 'true' });
    expect(((result.filters!.active as any) as FilterParam).value).toBe(true);
  });

  it('should coerce null values', () => {
    const result = parseRequest({ 'filter[deleted]': 'null' });
    expect(((result.filters!.deleted as any) as FilterParam).value).toBeNull();
  });

  it('should coerce comma-separated to array', () => {
    const result = parseRequest({ 'filter[status][in]': 'active,pending,done' });
    expect(((result.filters!.status as any) as FilterParam).value).toEqual(['active', 'pending', 'done']);
  });
  ...
  it('should handle URLSearchParams', () => {
    ...
    expect(((result.filters!.name as any) as FilterParam)).toEqual({ operator: 'eq', value: 'test' });
  ...
  describe('array operator normalization (in / notIn)', () => {
    it('should auto-wrap single string value to array for "in" operator', () => {
      ...
      expect(((result.filters!.status as any) as FilterParam)).toEqual({
        operator: 'in',
        value: ['processing'],
      });
    });
    ...
    it('should auto-wrap single numeric value to array for "in" operator', () => {
      ...
      expect(((result.filters!.id as any) as FilterParam)).toEqual({
        operator: 'in',
        value: [42],
      });
    });
    ...
    it('should produce empty array for empty string with "in" operator', () => {
      ...
      expect(((result.filters!.status as any) as FilterParam).operator).toBe('in');
      expect(Array.isArray(((result.filters!.status as any) as FilterParam).value)).toBe(true);
    });
  ...
  describe('nullary operator normalization (isNull / isNotNull)', () => {
    it('should discard value for isNull operator', () => {
      ...
      expect(((result.filters!.deletedAt as any) as FilterParam)).toEqual({
        operator: 'isNull',
        value: undefined,
      });
    });
  ...
  describe('between operator normalization', () => {
    it('should parse comma-separated pair as two-element array for "between"', () => {
      ...
      expect(((result.filters!.amount as any) as FilterParam)).toEqual({
        operator: 'between',
        value: [10, 100],
      });
    });
  ...
  describe('scalar operator passthrough', () => {
    it('should NOT wrap scalar for "eq" operator', () => {
      const result = parseRequest({ 'filter[status]': 'active' });
      expect(((result.filters!.status as any) as FilterParam).value).toBe('active');
    });

    it('should NOT wrap scalar for "gte" operator', () => {
      const result = parseRequest({ 'filter[amount][gte]': '100' });
      expect(((result.filters!.amount as any) as FilterParam).value).toBe(100);
    });

    it('should NOT wrap scalar for "like" operator', () => {
      const result = parseRequest({ 'filter[name][like]': '%john%' });
      expect(((result.filters!.name as any) as FilterParam).value).toBe('%john%');
    });
  ...
  describe('edge cases', () => {
    it('should handle empty string filter value', () => {
      ...
      expect(((result.filters!.status as any) as FilterParam)).toEqual({ operator: 'eq', value: '' });
    });

    it('should handle "undefined" as string value', () => {
      const result = parseRequest({ 'filter[status]': 'undefined' });
      expect(((result.filters!.status as any) as FilterParam)).toEqual({ operator: 'eq', value: 'undefined' });
    });
    ...
    it('should handle numeric string "1" as number', () => {
      ...
      expect(((result.filters!.count as any) as FilterParam).value).toBe(1);
    });
    ...
    it('should handle URL-encoded brackets in value (not key)', () => {
      ...
      expect(((result.filters!.note as any) as FilterParam)).toEqual({ operator: 'eq', value: '%5Bprocessing%5D' });
    });
    ...
    it('should handle trailing comma in in-operator value', () => {
      ...
      const val = ((result.filters!.status as any) as FilterParam).value;
    ...
    it('should handle filter value with spaces in comma list', () => {
      ...
      expect(((result.filters!.status as any) as FilterParam).value).toEqual(['a', 'b', 'c']);
    });

    it('should handle mixed casing (passes through as-is)', () => {
      const result = parseRequest({ 'filter[status][in]': 'Processing,SHIPPED' });
      expect(((result.filters!.status as any) as FilterParam).value).toEqual(['Processing', 'SHIPPED']);
    });
    ...
    it('should handle SQL injection attempt in filter value', () => {
      ...
      expect(((result.filters!.name as any) as FilterParam).value).toBe("'; DROP TABLE users; --");
    });
  });
  ...
  describe('URLSearchParams integration', () => {
    ...
    it('should handle the original bug case: single status with in operator', () => {
      ...
      expect(((result.filters!.status as any) as FilterParam).operator).toBe('in');
      expect(Array.isArray(((result.filters!.status as any) as FilterParam).value)).toBe(true);
      expect(((result.filters!.status as any) as FilterParam).value).toEqual(['processing']);
    });
  });
});
