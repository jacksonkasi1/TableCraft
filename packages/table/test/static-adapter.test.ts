...
const ITEMS: Item[] = [
  { id: 1, name: 'Apple',  price: 1.5,  category: 'fruit',    active: true,  createdAt: '2024-01-01' },
  { id: 2, name: 'Banana', price: 0.75, category: 'fruit',    active: true,  createdAt: '2024-01-02' },
  { id: 3, name: 'Carrot', price: 0.9,  category: 'vegetable', active: false, createdAt: '2024-01-03' },
  { id: 4, name: 'Date',   price: 3.0,  category: 'fruit',    active: true,  createdAt: '2024-01-04' },
  { id: 5, name: 'Eggplant', price: 2.2, category: 'vegetable', active: true, createdAt: '2024-01-05' },
];
...
describe('createStaticAdapter — pagination', () => {
  it('returns first page with correct meta', async () => {
    const adapter = createStaticAdapter(ITEMS);
    const result = await adapter.query({ ...BASE_PARAMS, page: 1, pageSize: 3 });
  ...
  it('returns last page with remaining items', async () => {
    const adapter = createStaticAdapter(ITEMS);
    const result = await adapter.query({ ...BASE_PARAMS, page: 2, pageSize: 3 });
...
describe('createStaticAdapter — sorting', () => {
  it('sorts ascending by string field', async () => {
    const adapter = createStaticAdapter(ITEMS);
    const result = await adapter.query({ ...BASE_PARAMS, sort: 'name', sortOrder: 'asc' });
  ...
  it('sorts descending by string field', async () => {
    const adapter = createStaticAdapter(ITEMS);
    const result = await adapter.query({ ...BASE_PARAMS, sort: 'name', sortOrder: 'desc' });
  ...
  it('sorts ascending by numeric field', async () => {
    const adapter = createStaticAdapter(ITEMS);
    const result = await adapter.query({ ...BASE_PARAMS, sort: 'price', sortOrder: 'asc' });
  ...
  it('sorts descending by numeric field', async () => {
    const adapter = createStaticAdapter(ITEMS);
    const result = await adapter.query({ ...BASE_PARAMS, sort: 'price', sortOrder: 'desc' });
  ...
  it('returns data unchanged when sort is empty string', async () => {
    const adapter = createStaticAdapter(ITEMS);
    const result = await adapter.query({ ...BASE_PARAMS, sort: '', sortOrder: 'asc' });
...
describe('createStaticAdapter — filters', () => {
  it('filters by exact string match (default operator)', async () => {
    ...
    expect(result.data).toHaveLength(3);
    result.data.forEach((r) => expect((r as Item).category).toBe('fruit'));
  });
  ...
  it('filters by gt numeric operator', async () => {
    ...
    result.data.forEach((r) => expect((r as Item).price).toBeGreaterThan(1.0));
  });
  ...
  it('filters by gte numeric operator', async () => {
    ...
    result.data.forEach((r) => expect((r as Item).price).toBeGreaterThanOrEqual(1.5));
  });
  ...
  it('filters by lt numeric operator', async () => {
    ...
    result.data.forEach((r) => expect((r as Item).price).toBeLessThan(1.0));
  });
  ...
  it('filters by lte numeric operator', async () => {
    ...
    result.data.forEach((r) => expect((r as Item).price).toBeLessThanOrEqual(1.5));
  });
  ...
  it('filters by neq operator', async () => {
    ...
    result.data.forEach((r) => expect((r as Item).active).toBe(true));
  });
