...
```tsx
import { DataTable, defineColumnOverrides, hiddenColumns } from '@tablecraft/table';
...
{% hint style="info" %}
`columnOverrides` receives an object with `value`, `row`, and `column` for maximum flexibility.
{% endhint %}
